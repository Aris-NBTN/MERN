    ...
    onEvent({ component }) {
      ...
      if (inputType.files[0]) {
        ...
        fileApi.file3D(formData)
          ...
          .catch(() => toastError("Có lỗi xảy ra"));
      }
    },
  });
...
const header = (editor) => {
  ...
  editor.Commands.add('addDropDownLink', {
    run(editor) {
      const selectedComponent = editor.getSelected();
      const liList = selectedComponent.find('.dropdown__submenu')[0];
      const countLi = liList.view.el.querySelectorAll('li').length
      ...
      if (countLi == 0) {
        ...
        selectedComponent.append(newListItem);
        addSubmenu();
      } else {
        addSubmenu();
      }
