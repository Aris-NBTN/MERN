...
export const FormatDayTimeWithHour = (data) => {
    const formattedDateTime = format(new Date(data), 'HH:mm - dd-MM-yyyy', { locale: vi });
    return formattedDateTime;
};

export const FormatDayTime = (data) => {
    let timeAgo = formatDistanceToNow(new Date(data), { locale: vi, addSuffix: true });
...
export const FormatPriceSaleCart = (carts) => {
    const total = carts.reduce((total, item) => total + item.price * (1 - item.sale / 100), 0);
    return total;
};

export const FormatPriceDiscount = (carts) => {
    const total = carts.reduce((total, item) => total + item.price * item.sale / 100, 0);
    return total;
};
...
export const FormatTag = ({ keywords }) => {
    const TagColors = [
        "geekblue", "blue", "cyan", "green", "lime", "purple", "magenta", "red", "volcano", "orange", "gold",
    ];
    return (
        <>
            {keywords?.map((item, index) => (
                <Tag className='my-1' color={TagColors[index % TagColors.length]} key={index}>
                    {item}
                </Tag>
            ))}
        </>
